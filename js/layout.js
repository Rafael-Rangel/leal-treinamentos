...
(function () {
  ...
  const LOGO_SRC = 'assets/logo-leal.png';
  ...
  const NAV = [
    ...
    { href: 'pacotes-promocionais.html', label: 'Pacotes', key: 'pacotes' },
  ...
  window.lealLogoMarkup = function (size) {

    const cls = size === 'xl' ? 'leal-logo--xl'

      : size === 'lg' ? 'leal-logo--lg'

      : size === 'md' ? 'leal-logo--md'

      : 'leal-logo--sm';

    const dim = size === 'xl' ? 120 : size === 'lg' ? 80 : size === 'md' ? 56 : 44;

    return `<span class="leal-logo-wrap"><img class="leal-logo ${cls}" src="${LOGO_SRC}" alt="Leal Treinamentos — RJ" width="${dim}" height="${dim}"></span>`;
  ...
  window.lealCtaMarkup = function (opts) {
    opts = opts || {};
    const href = opts.href || 'contato.html';
    return `
    <section class="cta-banner section-pad" aria-labelledby="leal-cta-heading">
      <div class="cta-banner__glow" aria-hidden="true"></div>
      <div class="cta-banner__inner reveal-scale px-6">
        <span class="cta-banner__label">Matricule-se</span>
        <h2 id="leal-cta-heading" class="font-display text-3xl md:text-5xl mb-6 uppercase">Pronto para Capacitar sua Equipe?</h2>
        <p class="text-on-dark-muted mb-10 max-w-xl mx-auto">Garanta conformidade, segurança e excelência com a referência em treinamentos do Rio de Janeiro.</p>
        <a class="btn-primary inline-flex items-center gap-2 px-12 py-5 text-lg" href="${href}">
          Matricule-se Agora
  ...
  window.renderLealLayout = function (activePage, opts = {}) {
    ...
    if (cta && opts.cta !== false) {
      cta.outerHTML = lealCtaMarkup({ href: opts.ctaHref || 'contato.html' });
    }
